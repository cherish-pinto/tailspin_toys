...
- Every exported function in `db/` and `src/lib/` must have a TSDoc/JSDoc
  comment. Describe its purpose, each parameter (including the injectable
  `db`), and its return value. Keep comments about intent or constraints; do
  not restate an obvious implementation.
