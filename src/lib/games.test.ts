...
describe('games data-access helpers', () => {
    ...
    it('filters games by one or more categories', async () => {
        ...
        const filtered = await getAllGames(db, { categoryIds: allCategories.map(({ id }) => id) });
    ...
    it('filters games by publisher', async () => {
        ...
        const [publisher] = await db
            ...
            .from(publishers)
            .where(eq(publishers.name, 'First Publisher'));

        const filtered = await getAllGames(db, { publisherId: publisher.id });
